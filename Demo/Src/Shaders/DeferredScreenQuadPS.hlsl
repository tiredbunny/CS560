...
Texture2D BrightColors : register(t4);

TextureCube IRMap : register(t5);
...
float4 main(VertexOut pin) : SV_TARGET
{
    ...
    float3 bloomColor = BrightColors.Load(sampleIndex).xyz;
    ...
    if (diff.a < 0.0f)
        return float4(albedo + bloomColor, 1.0f);
    ...
    Lo += (kD * albedo / 3.14159265359 + specular) * radiance * NdotL * posNshadow.w;
    Lo += bloomColor * NdotL;
