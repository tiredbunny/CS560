...
void Helpers::CreateGridXZ(std::vector<DirectX::VertexPositionNormalTexture>& vertices, std::vector<uint16_t>& indices,
...
void Helpers::CreateGridXY(std::vector<DirectX::VertexPositionNormalTexture>& vertices, std::vector<uint16_t>& indices,
	const uint16_t width, const uint16_t depth, uint16_t tessellation)
{
	vertices.clear();
	indices.clear();
	...
	for (int i = 0; i < tessellation; ++i)
	{
		...
		for (int j = 0; j < tessellation; ++j)
		{
			...
			vertices[rowIndex + j].position = XMFLOAT3(x, z, 0.0f);
			vertices[rowIndex + j].normal = XMFLOAT3(0.0f, 0.0f, -1.0f);
			vertices[rowIndex + j].textureCoordinate = XMFLOAT2(quadU * j, quadV * i);
		}
	}
