...
void M3DLoader::ReadVertices(std::ifstream& fin, UINT numVertices, std::vector<SkinnedVertex>& vertices)
{
	std::string ignore;
	...
	for (UINT i = 0; i < numVertices; ++i)
	{
		...
		fin >> ignore >> tangent.x >> tangent.y >> tangent.z >> tangent.w;
		fin >> ignore >> vertices[i].Normal.x >> vertices[i].Normal.y >> vertices[i].Normal.z;
		fin >> ignore >> vertices[i].TextureCoordinate.x >> vertices[i].TextureCoordinate.y;
	}
}
